...
def test_last_reviewed_at_was_bumped_for_this_material_change() -> None:
    ...
    which added the 18th contract (``MPA_SG_STATISTICS``). Bumped again to
    2026-08-10 by WO-047, which added the additive ``governance`` block
    (registry v0.4) to all 18 entries without changing any existing
    ``enabled``/``licence_status``/``qualification`` determination. The
    GDACS and TMD_CAP entries keep their own unchanged ``terms_reviewed_at``
    dates, which this bump does not touch.
    """
    registry = load_registry()
    assert registry["last_reviewed_at"] == "2026-08-10"
