...
- `schemas/lane.schema.json`, `schemas/logistics_event.schema.json`, `schemas/lane_assessment.schema.json` and `schemas/scenario_outlook.schema.json` carry the Ocean analytical model. All are mode-tagged rather than Ocean-hardcoded; the one exception is `logistics_event.schema.json`'s `event_type` enum, where two Ocean-worded values (`port_or_terminal_closure`, `canal_restriction`) have no exact fit for an Air/Land closure event and would need a small additive extension — a real, if small, schema change, not something already delivered (see `docs/air_land_extension_points.md` §1, verified against the real schema in `docs/bundle2_air_cargo_scope.md` §1).
