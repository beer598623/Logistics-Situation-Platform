...
- `schemas/lane.schema.json`, `schemas/logistics_event.schema.json`, `schemas/lane_assessment.schema.json` and `schemas/scenario_outlook.schema.json` carry the Ocean analytical model. All are mode-tagged rather than Ocean-hardcoded. `logistics_event.schema.json`'s `event_type` enum originally had two Ocean-worded values (`port_or_terminal_closure`, `canal_restriction`) with no exact fit for an Air/Land closure event; WO-035 closed that gap with a purely additive extension (`airspace_closure`, `terminal_or_facility_closure`) — see `docs/air_land_extension_points.md` §1, `docs/bundle2_air_cargo_scope.md` §1.
